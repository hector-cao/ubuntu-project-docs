...
{ref}`launchpad-specific-issues`
: Launchpad-specific build problems that usually cannot be reproduced locally or in PPA.
...
(launchpad-specific-issues)=
## Launchpad-specific issues

Sometimes a package builds successfully locally (and even in a PPA), but fails in Launchpad upload builds. This usually means a Launchpad-specific build step behaved differently from your local or PPA environment.
...
flowchart TD
  ...
  B -->|"wait(head==package2)"| D["dpkg-deb.pkgbinarymangler<br>Binary Package 2"]
  B -->|"wait(head==package3)"| E["dpkg-deb.pkgbinarymangler<br>Binary Package 3"]
  C --> F["package1"]
  D --> G["package2"]
  E --> H["package3"]
```

If `pkgbinarymangler` fails for one binary package, that package can remain in the lockfile queue and prevent other queued packages from being processed.
Because the Launchpad log pane is narrow, the failing package can scroll out of view quickly. In practice, you may only see repeated "waiting for lock" messages from other packages, which hides the original failure.

Example log excerpt:

```none
rage-hwe) ...
INFO: pkgstripfiles: waiting for lock (libvirt-common-hwe) ...
INFO: pkgstripfiles: waiting for lock (libvirt-daemon-driver-network-hwe) ...
INFO: pkgstripfiles: waiting for lock (libvirt-common-hwe) ...
INFO: pkgstripfiles: waiting for lock (libvirt-daemon-driver-storage-hwe) ...
INFO: pkgstripfiles: waiting for lock (libvirt-common-hwe) ...
INFO: pkgstripfiles: waiting for lock (libvirt-daemon-driver-storage-hwe) ...
INFO: pkgstripfiles: waiting for lock (libvirt-daemon-driver-network-hwe) ...
INFO: pkgstripfiles: waiting for lock (libvirt-daemon-driver-storage-hwe) ...
...
To reproduce this issue locally, install `pkgbinarymangler` in your build environment:

```none
$ apt install -y pkgbinarymangler
```